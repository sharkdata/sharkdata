...
from sharkdata_core import SharkdataAdminUtils, DatasetUtils, ResourcesUtils, SharkArchiveFileReader
...
class Command(BaseCommand):
    ...
    def _init_log(self):
        self.logfile_name = SharkdataAdminUtils().log_create(command='Update datasets and resources', user='auto')
    ...
    def _updateDb(self, data_type):
        ...
        if data_type == DataType.DATASETS:
            monitored_dir = MonitoredDirectory(DatasetUtils()._data_in_datasets, DATASET_FILE_PATTERN)
        ...
        for file_info in monitored_dir.get_new_files() + monitored_dir.get_modified_files():
            try:
                self._write_log('Loading file: %s ...' % (file_info.file_name))
                write_func(file_info)
            except Exception as e:
                error_counter += 1
                self._write_log('ERROR: Failed to load: ' + file_info.file_name + '. Error: ' + str(e))
            
        return error_counter
    
    def _deleteResourceFromDb(self, file_info):
        try:
            db_resource = resource_models.Resources.objects.get(resource_file_name=file_info.file_name)
            db_resource.delete()
    ...
    def _writeResourceToDb(self, file_info):
        ...
        resource_name, resource_type, encoding = self.splitResourceFilename(file_info.file_name)
        #
        try:
            resource_file = codecs.open(file_info.get_file_path(), 'r', encoding = encoding)
    ...
    def _deleteDatasetFromDb(self, file_info):
        try:
            db_dataset = datasets_models.Datasets.objects.get(dataset_name=file_info.name)
            db_dataset.delete()
    ...
    def _writeDatasetToDb(self, file_info):
        ...
        with SharkArchiveFileReader(file_info.file_name, file_info.directory_path) as zipreader:
            try:
        ...
        if ctd_profiles_table:
            ...
            ctdprofiles = ctdprofiles_models.CtdProfiles.objects.all().filter(dataset_file_name=file_info.file_name)
